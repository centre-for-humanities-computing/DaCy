...
perf = perf %>% 
  filter(!(model %in% unique(perf_w_dep$model) & augmenter == "No augmentation" & gpu == T)) %>%
...
perf = perf %>% select(-wall_time) %>% merge(., speed_perf, by=c("model", "gpu"))
...
no_dep = sum_perf_no_aug$model[sum_perf_no_aug$dep_las == 0]
```
...
score_t_test(mdl = "dacy_large", 
             ...
             data = perf %>% filter(!model %in% no_dep), 
             score="ents_f")
...
ent_perf = score_to_df(default = "No augmentation", data=perf %>% filter(!model %in% no_ent), score="ents_f")
...
pos_perf = score_to_df(default = "No augmentation", data=perf %>% filter(!model %in% no_pos), score="tag_acc")
...
las_perf = score_to_df(default = "No augmentation", data=perf %>% filter(!model %in% no_dep), score="dep_las")
