...
ent_per_tbl = ent_perf %>% arrange(conf_int) %>% 
  mutate(mean = paste(round(mean, 2)),
         ...
         string_value = if_else(is.na(sd), paste(mean, sep=""),
                                paste(mean,  " (", 
                                      format(round(sd, 2), digits = 2, nsmall=2), 
                                      ")", p_value_star, sep="")),
...
ent_per_tbl %>% 
  ...
  add_header_above(c(" " = 4, "Input length" = 2, "Names" = 5, 
                     "keystroke errors" = 3)) %>% 
  add_header_above(c(" " = 2, "Deterministic Augmentation" = ncol(ent_per_tbl)-2 - length(bootstrapped_aug), 
...
pos_per_tbl %>% 
  ...
  add_header_above(c(" " = 2, "Deterministic Augmentation" = 4, "Stochastic Augmentations" = 3)) %>% 
