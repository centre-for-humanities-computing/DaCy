...
def accuracy_report(tags_true, tags_pred):
    ...
    for label in labels:
        correct_tags[label] = [
            t == p for t, p in zip(tags_true, tags_pred) if t == label
        ]
        acc = (
            round(sum(correct_tags[label]) / len(correct_tags[label]) * 100, 2)
            if len(correct_tags[label]) > 0
            else 0
        )
    ...
    micro_acc = round(
        sum(sum(correct_tags[l]) for l in correct_tags) / total_examples * 100, 2
    )
    ...
    macro_acc = round(
        sum(
            [
                sum(correct_tags[l]) / len(correct_tags[l])
                for l in labels
                if len(correct_tags[l]) > 0
            ]
        )
        / len(labels)
        * 100,
...
def dependency_report(deps_true, deps_pred):
    ...
    labels = sorted(
        list(
            set(
                [r for s in deps_true for (r, _) in s]
                + [r for s in deps_pred for (r, _) in s]
            )
        )
    )
    ...
    for l in labels:
        ...
        tab.append(
            [
                l,
                round(la_score_per_label[-1] * 100, 2),
                round(uas_score_per_label[-1] * 100, 2),
                round(las_score_per_label[-1] * 100, 2),
                correct_deps[l]["total"],
            ]
        )
    tab.append([""] * 5)
    ...
    micro_la = round(
        sum(correct_deps[l]["rel"] for l in correct_deps) / total_examples * 100, 2
    )
    micro_uas = round(
        sum(correct_deps[l]["head"] for l in correct_deps) / total_examples * 100, 2
    )
    micro_las = round(
        sum(correct_deps[l]["both"] for l in correct_deps) / total_examples * 100, 2
    )
    ...
    return tabulate(
        tab,
        headers=headers,
        colalign=["right", "decimal", "decimal", "decimal", "right"],
    )
